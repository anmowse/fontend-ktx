...
const UserForm = ({ user, onSubmit, onCancel }) => {
  ...
  useEffect(() => {
    ...
    // Reset lỗi khi chuyển đổi giữa chế độ edit và thêm mới
    setErrors({});
  }, [user]);
  ...
  const validateForm = () => {
    ...
    // Kiểm tra số điện thoại - phải có chính xác 10 số
    if (!formData.phone.trim()) {
      newErrors.phone = "Số điện thoại không được để trống";
    } else if (!/^[0-9]{10}$/.test(formData.phone.replace(/\s/g, ""))) {
      newErrors.phone = "Số điện thoại phải có chính xác 10 số";
    }
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
          ...
          <div className="sm:col-span-3">
            ...
            >
              {isEditMode
                ? "Mật khẩu mới (không điền nếu không đổi)"
                : "Mật khẩu"}
            </label>
            <div className="mt-1">
              <input
